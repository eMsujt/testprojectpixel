...
import com.skyblock.plugin.item.ItemRegistry.Rarity;
...
/**
 * Singleton manager of custom item definitions loaded from {@code items.yml}.
 *
 * <p>On {@link #load(JavaPlugin)} the bundled {@code items.yml} is copied out of
 * the jar on first run and every key under the {@code items} section is parsed
 * into an {@link ItemDefinition} held in memory and looked up by id.</p>
 */
