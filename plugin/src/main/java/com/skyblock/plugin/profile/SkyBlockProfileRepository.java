 ...
 */
public final class SkyBlockProfileRepository {

    private static final SkyBlockProfileRepository INSTANCE = new SkyBlockProfileRepository();

    private SkyBlockProfileRepository() {}

    public static SkyBlockProfileRepository getInstance() {
        return INSTANCE;
    }
    ...
    public CompletableFuture<Void> saveAsync(Plugin plugin, SkyBlockProfile profile) {
        ...
        cfg.set("showSkillNotifications", profile.isShowSkillNotifications());
        ...
        cfg.set("ownedPets", profile.getOwnedPets());
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                if (file.exists()) {
                    ...
                    profile.setShowSkillNotifications(cfg.getBoolean("showSkillNotifications", true));
                    profile.setShowCollectionNotifications(cfg.getBoolean("showCollectionNotifications", true));
                    profile.setShowPetNotifications(cfg.getBoolean("showPetNotifications", true));
                    ...
                    List<?> ownedPets = cfg.getList("ownedPets");
                    if (ownedPets != null) {
                        for (Object pet : ownedPets) {
                            if (pet instanceof String) {
                                profile.addOwnedPet((String) pet);
                            }
                        }
                    }
