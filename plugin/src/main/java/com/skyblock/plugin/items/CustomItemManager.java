...
/**
 * YAML-driven manager of custom {@link SkyBlockItem} definitions loaded from
 * {@code plugins/SkyBlock/items/custom_items.yml}.
 *
 * <p>Each item is defined under the top-level {@code items} section, keyed by
 * its id and carrying the material it renders as, its rarity tier, a display
 * name and its stat bonuses. The bundled default is copied out of the jar on
 * first run. Loaded items are held in memory and looked up by their id.</p>
 */
public final class CustomItemManager {

    private static final String RESOURCE = "items/custom_items.yml";
