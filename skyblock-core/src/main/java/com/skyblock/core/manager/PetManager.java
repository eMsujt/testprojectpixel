...
public final class PetManager {
    ...
    /**
     * Canonical per-level pet XP increments (NotEnoughUpdates pets.json "pet_levels");
     * index 0 = the cost to go from level 1 to 2. Each rarity reads a window of this
     * shared 119-entry array starting at its {@link #PET_RARITY_OFFSET}.
     */
    private static final long[] MASTER_PET_XP = {
    ...
    /** Master-array offset per rarity (NEU "pet_rarity_offset"). MYTHIC mirrors LEGENDARY. */
    private static final Map<String, Integer> PET_RARITY_OFFSET = Map.of(
            "COMMON", 0, "UNCOMMON", 6, "RARE", 11, "EPIC", 16, "LEGENDARY", 20, "MYTHIC", 20);

    static {
        Map<String, long[]> m = new HashMap<>();
        for (Map.Entry<String, Integer> e : PET_RARITY_OFFSET.entrySet()) {
            m.put(e.getKey(), petCumulative(e.getValue()));
        }
        PET_XP_TABLE = Collections.unmodifiableMap(m);
    }

    /**
     * Builds the cumulative pet-XP threshold table for a rarity offset: index {@code i}
     * is the total XP to reach level {@code i+2} (index 98 = total to reach level 100),
     * matching the indexing {@link #computeLevel} expects.
     */
    private static long[] petCumulative(int offset) {
        long[] table = new long[MAX_LEVEL];
        long total = 0;
        for (int i = 0; i < MAX_LEVEL - 1; i++) {
            total += MASTER_PET_XP[offset + i];
            table[i] = total;
        }
