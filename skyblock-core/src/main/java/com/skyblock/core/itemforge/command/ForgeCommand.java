package com.skyblock.core.itemforge.command;
...
/**
 * Handles the {@code /itemforge} command.
 *
 * <p>Subcommands:
 * <ul>
 *   <li>{@code /itemforge list}            — list all available forge recipes</li>
 *   <li>{@code /itemforge start <recipe>}  — begin forging a recipe</li>
 *   <li>{@code /itemforge status}          — check progress of the active forge job</li>
 *   <li>{@code /itemforge collect}         — collect a completed forge job</li>
 *   <li>{@code /itemforge cancel}          — cancel the active forge job</li>
 * </ul>
 * </p>
 */
public final class ForgeCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage("Usage: /itemforge <list|start|status|collect|cancel>");
            return true;
        }
        ...
        switch (args[0].toLowerCase()) {
            ...
            default        -> player.sendMessage("Unknown subcommand. Usage: /itemforge <list|start|status|collect|cancel>");
        }
        return true;
    }
    ...
    private void handleList(Player player) {
        if (forgeManager.getRecipes().isEmpty()) {
            player.sendMessage("No item forge recipes available.");
            return;
        }
        player.sendMessage("=== Item Forge Recipes ===");
        forgeManager.getRecipes().values().stream()
                ...
                .forEach(recipe -> {
                    ...
                    player.sendMessage(String.format("[%s] %s -> %dx %s (%s) | %ds | Needs: %s",
                            recipe.getId(),
                            recipe.getDisplayName(),
                            recipe.getOutputAmount(),
                            recipe.getOutputItem().id(),
                            recipe.getOutputItem().rarity().name(),
    ...
    private void handleStart(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage("Usage: /itemforge start <recipe>");
            return;
        }
        String recipeId = args[1].toLowerCase();
        if (forgeManager.getRecipe(recipeId) == null) {
            player.sendMessage("Unknown recipe: " + recipeId + ". Use /itemforge list to see available recipes.");
            return;
        }
        try {
            forgeManager.startForge(player.getUniqueId(), recipeId, System.currentTimeMillis());
            ItemForgeManager.ItemForgeRecipe recipe = forgeManager.getRecipe(recipeId);
            player.sendMessage("Started forging: " + recipe.getDisplayName()
                    + ". Time remaining: " + formatDuration(recipe.getDurationSeconds()));
        } catch (IllegalStateException e) {
            player.sendMessage("You already have an active forge job. Use /itemforge status to check it.");
        }
    }

    private void handleStatus(Player player) {
        ItemForgeManager.ItemForgeJob job = forgeManager.getActiveJob(player.getUniqueId());
        if (job == null) {
            player.sendMessage("You have no active forge job. Use /itemforge start <recipe> to begin one.");
            return;
        }
        long nowMillis = System.currentTimeMillis();
        if (job.isComplete(nowMillis)) {
            player.sendMessage("Forge complete: " + job.getRecipe().getDisplayName()
                    + " is ready! Use /itemforge collect to claim it.");
        } else {
    ...
    private void handleCollect(Player player) {
        try {
            ItemForgeManager.ItemForgeJob job = forgeManager.collectForge(player.getUniqueId(), System.currentTimeMillis());
            player.sendMessage("Collected " + job.getRecipe().getOutputAmount() + "x "
                    + job.getRecipe().getOutputItem().id()
                    + " [" + job.getRecipe().getOutputItem().rarity().name() + "] from forging "
                    + job.getRecipe().getDisplayName() + "!");
        } catch (IllegalStateException e) {
            player.sendMessage(e.getMessage().contains("not yet complete")
                    ? "Your forge job is not complete yet. Use /itemforge status to check progress."
