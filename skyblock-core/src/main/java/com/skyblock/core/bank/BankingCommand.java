package com.skyblock.core.bank;

import org.bukkit.command.Command;
import org.bukkit.command.CommandSender;
import org.bukkit.command.TabExecutor;

import java.util.List;

/**
 * @deprecated Use {@link com.skyblock.core.bank.command.BankingCommand} instead.
 */
@Deprecated
public final class BankingCommand implements TabExecutor {
    private final com.skyblock.core.bank.command.BankingCommand delegate;
    public BankingCommand(com.skyblock.core.manager.BankManager bankingManager) {
        this.delegate = new com.skyblock.core.bank.command.BankingCommand(bankingManager);
    }
    @Override public boolean onCommand(CommandSender s, Command c, String l, String[] a) { return delegate.onCommand(s, c, l, a); }
    @Override public List<String> onTabComplete(CommandSender s, Command c, String al, String[] a) { return delegate.onTabComplete(s, c, al, a); }
}
