...
class MenuIntegrationTest {
    ...
    class WardrobeMenuTests {
        ...
        @Test
        void title_isWardrobe() {
            assertEquals("§eWardrobe", new WardrobeMenu(PLAYER).getTitle());
        }

        @Test
        void rows_isSix() {
            assertEquals(6, new WardrobeMenu(PLAYER).getRows());
        }
        ...
        @Test
        void firstSlot_isEighteen() {
            assertEquals(18, WardrobeMenu.FIRST_SLOT);
        }

        @Test
        void maxOutfits_isNine() {
            assertEquals(9, WardrobeManager.MAX_OUTFITS);
        }
        ...
        @Test
        void saveOutfit_roundTrip_outfitIsRetrievable() {
            WardrobeManager manager = WardrobeManager.getInstance();
            boolean saved = manager.saveOutfit(PLAYER, WardrobeSlot.SLOT_1, new org.bukkit.inventory.ItemStack[4]);
            assertTrue(saved);
            assertNotNull(manager.getOutfit(PLAYER, WardrobeSlot.SLOT_1));
        }

        @Test
        void getActiveArmorSet_nullForFreshPlayer() {
            assertNull(WardrobeManager.getInstance().getActiveArmorSet(PLAYER));
        }

        @Test
        void equip_setsActiveArmorSet() {
            WardrobeManager manager = WardrobeManager.getInstance();
            manager.saveOutfit(PLAYER, WardrobeSlot.SLOT_1, new org.bukkit.inventory.ItemStack[4]);
            manager.equip(PLAYER, WardrobeSlot.SLOT_1);
            assertEquals(WardrobeSlot.SLOT_1.name(), manager.getActiveArmorSet(PLAYER));
        }
    }
}
